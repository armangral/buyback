  ...
  >
    ...
    <div className="p-4">
      ...
      <p className="text-sm text-gray-500 mb-2">
        {device.brand} • {device.year_of_purchase}
      </p>
      {/* Add Verification Status Here */}
      <p
        className={`text-sm font-bold ${
          device.approved_product ? "text-green-600" : "text-red-600"
        }`}
      >
...
const FeaturedDevice = ({ device, onClick }) => (
  <div className="bg-white shadow-lg rounded-lg overflow-hidden mb-8 border border-gray-200">
    <div className="md:flex">
      ...
      <div className="p-8">
        <h2 className="text-xl font-bold text-gray-800">{device.model}</h2>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            className="ml-4 px-4 py-2 bg-purple-600 text-white font-bold rounded-md hover:bg-purple-700 transition-colors"
            onClick={() => onClick(device)}
          >
            View Details
          </button>
        ...
        <div className="flex">
          ...
          <div className="ml-6 flex-1">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
              ...
              <p>
                <strong>Year:</strong> {device.year_of_purchase}
              </p>
              <p>
                <strong>Condition:</strong> {device.condition} (
                {device.overall_rating})
              </p>
              <p>
                <strong>Storage:</strong> {device.storage}
              </p>
              <p>
                <strong>Serial Number:</strong> {device.imei}
              </p>
              <p>
                <strong>Defects:</strong>{" "}
                {device.defects.length > 0 ? device.defects.join(", ") : "None"}
              </p>
              {/* Add Verification Status Here */}
              <p
                className={`font-bold ${
...
const BuybackMarketplace = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-white min-h-screen">
      ...
      {featuredDevice && (
        <FeaturedDevice device={featuredDevice} onClick={setSelectedDevice} />
      )}
...
export default BuybackMarketplace;
