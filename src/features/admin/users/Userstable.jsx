  ...
  if (loading) {
    return (
      <Flex justify="center" mt="5">
        ...
        <Text mt="3" color="gray.600">Loading users...</Text>
      </Flex>
    );
  }

  return (
    <Box bg="gray.50" borderRadius="xl" p={8} boxShadow="md" mt={8}>
      <VStack spacing={4} align="start">
        ...
        <Table variant="simple" size="sm">
          <Thead>
            <Tr>
              <Th color="gray.600" fontSize="sm">Email</Th>
              <Th color="gray.600" fontSize="sm">UID</Th>
              <Th color="gray.600" fontSize="sm">Name</Th>
              <Th color="gray.600" fontSize="sm">CNIC</Th>
              <Th color="gray.600" fontSize="sm">Verified</Th>
            </Tr>
          </Thead>
          <Tbody>
            {users.map((user, index) => (
              <Tr key={index}>
                ...
                <Td fontSize="sm" color="gray.700">{user.uid}</Td>
                <Td fontSize="sm" color="gray.700">{user.name}</Td>
                <Td fontSize="sm" color="gray.700">{user.cnic}</Td>
                <Td>
                  {user.isVerified === "Yes" ? (
